...
- Improvements of existing code/functionality
      - Higher priority
            - Make tests more stable w.r.t. timing, use looping instead of sleep
            - When a test fails, other tests after it should not fail. Move all stuff to "before test" from "after test".
      - Lower priority
