...
- erlang-language-platform related: the LSP server can send requests to the client. Such requests are incorrectly decoded as responses, and the expected request id is too big when the client later sends and waits for response.
...
- [Functionality] Adjustable =MAX_LOG_ENTRY_LEN_BYTES=, because when debugging, sometimes the full message needs to be seen.
