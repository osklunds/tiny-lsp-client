...
Note: over time, it turns out that even with a Docker image, it's problematic to keep the servers from starting at all, and keep the test stable due to small changes in behavior. Once tests start to break, I might consider only keeping the most core serves, i.e. =clangd= and =rust-analyzer=.
...
In general, there are probably many "leaky pipes" with bugs that can cause crashes/freezes. However, I focus on fixing such cases, and issues in general, as I encounter them. That automatically prioritizes what to work on.

** Bugs

*** High priority

- =jdtls= doesn't start, so =jdtls= tests are broken.
- =eldoc= for =jdtls= doesn't work after upgrading =jdtls= from 1.46.0 to 1.56.0.

*** Low priority

*** Might never do

- When =jdtls= failed to start, a line with a log file path was printed when running =jdtls= directly in the terminal but was not shown in the STDERR log.
...
- erlang-language-platform: sometimes completion respones doesn't decode successfully. The result field is None.
...
- [Functionality] Support lsp rename
- [Robustness] Timeout when starting server. If no initialize response after custom time, default 5 seconds, kill the server and consider start failed.

*** Low priority

- [Test] Switch from Tumbleweed to Debian. Even if a commit hash is specified as image version, on old commits, that might still download packages later than the commit assumed, meaning tests won't pass if you checkout an old commit.
      - But switching is not problematic. Compiling manually is not an option, it's time consuming and more error prone, and e.g. libgccjit version from the package manager still needs to match. Ubuntu might have more recent versions than Debian. But maybe still not the latest. And then Ubuntu might not be as stable as Debian. So, this requires some more consideration before going ahead.
- [Functionality] Signature help in eldoc
- [Code quality] Generalized FromLisp for union types. Maybe it's possible to check if the lisp value is of correct type. Maybe a can_from_lisp function on the FromLisp trait? Maybe emacs-module-rs does it in a smart way?
- [Code quality] Clean up server.rs by having smaller functions (like read_header) and having a flatter sructure since break can return early.
- [Performance] Remove duplicate completions in Rust code.
- [Performance] Improve the =servers::with_servers()= API to not call =server::is_working()= on all servers, only for the =ServerKey= requested, to prevent linear slow down when many servers.
- [Robustness] Prevent crashes in rust code due to bad user settings, e.g. empty server cmd.
- [Robustness] In general, ensure no bad args sent to rust. e.g. stop-server with "path" because doesn't start with /
- [Robustness] Time out for all calls towards rust. Or at least, make sure all existing calls return quickly and can't freeze. Starting a new server can freeze if the server cmd never sends an initialize response.
- [Test] Consider to use zypper's ghc and cabal.
- [Test] Test when server cmd of LSP server not found.
- [Test] Log tests
      - Unit tests if possible
      - test what happens if =tlc-log-file= is never set
- [Test] Use clangd instead of rust-analyzer inside rust unit tests
- [Test] More unit tests of things in general, where it makes sense
- [Test] Test error during xref or capf
- [Test] Some tests are unstable, especially when running all
- [Test] Test coverage for "(? ms)" in log, i.e. id not found
- [Test] In general, check that all variants of all messages are tested. For example, all of label, insertText and textEdit of CompletionItem. Can do it by counting in log file.
- [Test] Test too big id in response from server.
- [Investigation] consider if utf-16 actions need to be taken for xref, like =eglot= seems to do
- [Investigation] Error cases
...
- [Investigation] Understand how LSP processes are killed even if not same process group. It happens even when wrong process id in initialize message.
- [Investigation] Understand bounds and symbol better for capf
- [Investigation] Understand commit chars, range, etc from LSP better. Check what lsp-mode and eglot do
- Completion
      - [Functionality] If interrupted, send dabbrev
      - [Functionality] If interrupted, keep calculating and use that as next last result. So like the "cached async" but it keeps on refreshing and doesn't do it just once in the beginning.
      - [Investigation] For async, measure how much time it takes to do everything except while-no-input. And with. And try to skip it for debug purposes
      - [Investigation] Understand how while-no-input, sit-for, and sleep-for interact.
      - [Investigation] Understand how company completes not just from prefix when used with LSP
      - [Investigation] Understand how lsp-mode and eglot handle async and cached completions

*** Might never do

- [Functionality] For e.g. =jdtls= and =hls= make it possible to jump to library files
- [Functionality] Multiple servers per buffer, could be relevant for HTML, CSS and Javascript
...
- [Functionality] erlang-language-platform related: take common part of insertText and label if insertTextFormat is snippet, becuase erlang-language-platform doesn't seem to honor that client only supports plaintext format.
      - Solve ad-hoc instead, e.g. keybind to quickly replace ${1:arg} similar to =ciw= in vim.
- [Performance] Support utf-32 line pos, that =eglot= has as preffered, since it has better performance than utf-16.
      - Might never do since small gain for lots of complexity.
- [Performance] Consider caching didChange like eglot.
      - Might never do since small gain for lots of complexity.
- [Correctness] Keep track of server capabilities.
