...
- Bugs
      - Higher priority
            - =haskell-mode='s indentation causes asserts to fail in =tlc-after-change=.
                  - Need to have the fallback to full sync.
                  - For edit-tests need to check number of =didChange= as well as their type.
      - Lower priority
      - Hard to reproduce
